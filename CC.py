...
class Sign_up_page(Frame):
    ...
    def sign_up(self, master):
        if newuser.get() == '':
            ...
            if newpassword.get() == '':
                messagebox.showerror('Ошибка ввода',
                                     'Поле ввода пароля не может быть пустым.')
            else:
                with open(file_name, 'a', encoding='utf-16') as f:
                    res = newuser.get() + ' ' + encrypt(newpassword.get(), cesar(newuser.get())) + '\n'
                    f.write(res)
...
class Game_page():
    ...
    def __init__(self, master):
        ...
        game_window.wm_attributes("-topmost", 1) #устанавливаем окно поверх другого
    ...
    def new_game(self):  # начинаем новую игру
        ...
        #инициализируеv начальное состояние игрового поля
    ...
    def comp_move(self):  # !!!
        ...
        if comp_moves_f_list:  # проверяем наличие доступных ходов
            count_moves = len(comp_moves_f_list)  # количество доступных ходов
            ...
            for i in range(len_move - 1):
                ...
                self.turn(1, comp_moves_f_list[t_move][i][0], comp_moves_f_list[t_move][i][1],
                     comp_moves_f_list[t_move][1 + i][0], comp_moves_f_list[t_move][1 + i][1]) # выполнение отдельного шага хода
    ...
    def check_comp_move(self, tur, n_list, moves_list):  # проверяем оставшиеся ходы
        global scene
        global comp_moves_f_list
...
if __name__ == "__main__":
    ...
    # board.bind("<Motion>", mouse_pos_from)  # движение мышки по полю
    # board.bind("<Button-1>", mouse_pos_to)  # нажатие левой кнопки
'''
import os
from tkinter import *
import random
